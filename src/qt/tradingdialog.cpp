...
void tradingDialog::UpdaterFunction(){
    //EVOst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::BuyEVO(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::SellEVO(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=EVO";
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       ui->EVOAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       ...
                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->EVOBalanceLabel,*ui->EVOAvailableLabel,*ui->EVOPendingLabel, QString::fromUtf8("EVO"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputEVO->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::on_Sell_Max_Amount_clicked()
{
    ...
    ui->UnitsInputEVO->setText(str.number(AvailableEVO,'i',8));
}
...
void tradingDialog::on_BuyEVO_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyEVO(Order,Quantity,Rate);
...
void tradingDialog::on_SellEVOBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputEVO->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellEVO(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputEVO_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
